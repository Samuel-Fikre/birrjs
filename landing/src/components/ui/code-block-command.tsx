...
import { TextQuote, Terminal } from "lucide-react";
...
export function CodeBlockCommand({
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl bg-code shadow-lg border border-white/10">
      <Tabs
        className="gap-0"
        ...
        <ScrollArea.Root className="w-full pr-10 shadow-[inset_0_-1px_0_0] shadow-border">
          ...
          >
            ...
            {tabsFiltered.map(([key]) => {
              return (
                <TabsTrigger key={key} className="h-8 rounded-lg p-0 px-3 font-mono" value={key}>
                  {key}
                </TabsTrigger>
              );
            })}
        ...
        {tabsFiltered.map(([key, value]) => {
          return (
            <TabsContent key={key} value={key}>
              <pre
                data-pm={key}
                className="group/tabs-content-pre overscroll-x-contain p-5 leading-7 not-data-[pm=prompt]:overflow-x-auto"
              >
                <code
                  data-slot="code-block"
                  data-language="bash"
                  className="font-mono text-base/none text-white/85 group-data-[pm=prompt]/tabs-content-pre:whitespace-normal"
                >
