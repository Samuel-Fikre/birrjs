...
| `trialing` | Free trial active, `trialEndsAt` set |
...
| `trialing` | Within trial period | `trialing` |
| `trialing` | `trialEndsAt` passed | `expired` |
...
<Steps>
  ...
  Call `subscribe()` with the target plan ID. BirrJS creates a subscription in `pending` status and initializes a transaction with the payment provider. For paid plans, the result includes a `checkoutUrl` (redirect providers like Chapa) or `paymentInstructions` (receipt-based providers like Vodit).
  ...
  // or result.paymentInstructions → show bank channels (Vodit)
  ...
  // result.trialEndsAt → trial started
  ```
  ...
  If the plan has `trialDays` configured and the `@birrjs/trial` plugin is registered, the subscription is created in `trialing` status with a `trialEndsAt` date. The user can access features during the trial.

  When the trial ends or the user chooses to pay early, the subscription transitions to `active`. Entitlements are either reset or carried over based on `resetOnTrialConversion`.

  <Tabs items={["Chapa", "Vodit"]}>
    <Tab value="Chapa">

    ```ts
    // User clicks "Pay now" → Chapa checkout
    const { checkoutUrl } = await client.subscribe({ planId: "pro" });
    window.location.href = checkoutUrl;
    // After payment, Chapa webhook activates the subscription
    ```
    ...
    ```ts
    // User clicks "Pay now" → get bank channels
    const { paymentInstructions } = await client.subscribe({ planId: "pro" });
    // User pays and submits receipt
    await client.verifyReceipt({ subscriptionId, receiptUrl });
    ```

    </Tab>
  </Tabs>

  Stale trialing subscriptions are marked `expired` by the expiry sweep cron (every 10 minutes by default).
  </Step>
...
| `useTrial` | No | `true` to start a free trial. Requires plan with `trialDays` and the `@birrjs/trial` plugin. |
| `fingerprint` | No | Browser fingerprint for trial abuse prevention. Required when `useTrial: true`. Collect via `@birrjs/fingerprint`. |

## Result Shape

| Field | Description |
|-------|-------------|
| `checkoutUrl` | The payment provider's checkout URL. Redirect the user here to complete payment (Chapa). |
| `paymentInstructions` | Bank account details for manual payment (Vodit). |
| `trialEndsAt` | ISO date string when the trial period ends. Present when a trial subscription was created or returned. |
| `subscriptionId` | The ID of the created subscription record. |
| `customerId` | The customer ID the subscription was created for. |

If `checkoutUrl` is set, the subscription stays in `pending` until the provider confirms payment via webhook. If `paymentInstructions` is set, the user pays manually and submits their receipt via `verifyReceipt`. If `trialEndsAt` is set, the subscription is in `trialing` status with free access until that date.
...
| Expiry | Every 10 min | Marks expired `active` and `trialing` subscriptions as `expired` |
