...
All 8 client-accessible methods:

| Method | Input | Output |
|--------|-------|--------|
| `subscribe` | `{ planId, useTrial?, fingerprint? }` | `{ checkoutUrl?, paymentInstructions?, trialEndsAt?, subscriptionId, customerId }` |
| `verifyReceipt` | `{ subscriptionId, receiptUrl }` | `{ success, subscriptionId, alreadyActive? }` |
...
- **`subscribe`** — Creates a subscription. Redirect providers (e.g. Chapa) return a `checkoutUrl` for checkout. Manual verification providers (e.g. Vodit) return `paymentInstructions` with bank accounts for the customer to pay manually. Pass `useTrial: true` + a browser `fingerprint` to start a free trial (requires plan with `trialDays` and the `@birrjs/trial` plugin). The customer is resolved from the current session — do not pass a `customerId`.
- **`verifyReceipt`** — Validates a payment receipt via the provider (Vodit). After the user pays to a bank account and gets a receipt URL, call this to verify the transaction. Checks receipt amount >= plan price and activates the subscription on success. Returns `alreadyActive: true` if the subscription is already active (idempotent).
  ...
  function SubscribeButton({ planId }: { planId: "free" | "pro" }) {
    return (
      <button
        onClick={async () => {
          const result = await client.subscribe({ planId });
          // Display result.paymentInstructions.amount and channels to the user
        }}
      >
        Subscribe to {planId}
      </button>
