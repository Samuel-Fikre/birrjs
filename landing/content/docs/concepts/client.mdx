...
| Method | Input | Output |
|--------|-------|--------|
| `subscribe` | `{ planId }` | `{ checkoutUrl?, paymentInstructions?, subscriptionId, customerId }` |
...
- **`subscribe`** — Creates a subscription. Redirect providers (e.g. Chapa) return a `checkoutUrl` for checkout. Manual verification providers (e.g. Vodit) return `paymentInstructions` with bank accounts for the customer to pay manually. The customer is resolved from the current session — do not pass a `customerId`.
