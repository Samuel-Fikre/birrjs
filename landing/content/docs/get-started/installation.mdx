---
title: Installation
description: Install BirrJS, configure a provider, and mount the route handler in your app.
---

<Callout type="info">
  **Want to skip the manual setup?** Run `npx @birrjs/cli init` to scaffold the full configuration with framework detection, route handler, and plan templates. See the [CLI reference](/docs/concepts/cli).
</Callout>
...
<Tabs items={["Chapa", "Vodit"]}>
  ...
  <Tabs items={["pnpm", "npm", "yarn", "bun"]}>
    <Tab value="pnpm">
    ```bash
    pnpm add @birrjs/core @birrjs/vodit
    ```
    </Tab>
    <Tab value="npm">
    ```bash
    npm install @birrjs/core @birrjs/vodit
    ```
    </Tab>
    <Tab value="yarn">
    ```bash
    yarn add @birrjs/core @birrjs/vodit
    ```
    </Tab>
    <Tab value="bun">
    ```bash
    bun add @birrjs/core @birrjs/vodit
    ```
    </Tab>
  </Tabs>
  </Tab>
...
BirrJS uses provider adapters for payment processing. Choose your provider:
...
<Tabs items={["Chapa", "Vodit"]}>
  ...
  ```ts title="src/lib/birrjs.ts"
  import { createBirr } from "@birrjs/core";
  import { vodit } from "@birrjs/vodit"; // [!code highlight]

  export const birrjs = createBirr({
    provider: vodit({ // [!code highlight]
      apiKey: process.env.VODIT_API_KEY!, // [!code highlight]
      channels: [ // [!code highlight]
        { // [!code highlight]
          type: "telebirr", // [!code highlight]
          value: process.env.VODIT_TELEBIRR_ACCOUNT!, // [!code highlight]
          name: "Abebe", // Must match your bank account name exactly // [!code highlight]
        }, // [!code highlight]
      ], // [!code highlight]
    }),
  });
  ...
  <Callout type="warning">
    **The `name` field is required** — Vodit uses it to verify the receipt's recipient matches your account. Use the name as registered at the bank (first name is fine). See the [Vodit provider page](/docs/providers/vodit) for details.
  </Callout>
...
Want SMS notifications on subscription events? Add the [Afromessage plugin](/docs/plugins/notification/afromessage) — or build your own with the [Your First Plugin](/docs/guides/your-first-plugin) guide.
